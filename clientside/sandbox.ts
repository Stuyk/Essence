...
// FATHER CODE
// ===============================
function addFather(contentHolder: ContentHolder) {
    var item: ContentItem = contentHolder.addItem("Father");
    item.Description = "Which 'Father' face type would you like to blend."
    for (var i = 0; i < 46; i++) {
        var subitem: ContentItem = item.addVariedContentItem(contentHolder, i.toString());
        setupFatherFaceFunction(subitem, i);
    }
    getCurrentFatherValue("ESS_Father", item);
}
function getCurrentFatherValue(syncString: string, item: ContentItem) {
...
function setupFatherFaceFunction(subitem: ContentItem, id: number) {
    // Select Function
    var selectHolder: FunctionHolder = subitem.getSelectFunction();
    selectHolder.Function = setFatherFace;
    selectHolder.Args = [id];
    subitem.SelectFunction = selectHolder;
    // Hover Function
    var hoverHolder: FunctionHolder = subitem.getHoverFunction();
...
// Mother CODE
// ===============================
function addMother(contentHolder: ContentHolder) {
    var item: ContentItem = contentHolder.addItem("Mother");
...
function getCurrentMotherValue(syncString: string, item: ContentItem) {
    if (API.hasEntitySyncedData(API.getLocalPlayer(), syncString)) {
        item.CurrentSelection = API.getEntitySyncedData(API.getLocalPlayer(), syncString);
...
function setupMotherFaceFunction(subitem: ContentItem, id: number) {
    // Select Function
    var selectHolder: FunctionHolder = subitem.getSelectFunction();
...
function updateMotherFace(args) {
    API.returnNative("SET_PED_HEAD_BLEND_DATA", Enums.NativeReturnTypes.UInt, API.getLocalPlayer(), args[0], father, 0, motherSkin, fatherSkin, 0, parseFloat(faceBlend), parseFloat(skinBlend), 0, false);
}
